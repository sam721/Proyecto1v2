...
void main(void) {
	light lights[3] = light[3](light(lightPos1, lightDir1, lightAmb1, lightDif1, lightSpec1, spotDir1, spotExp1, spotCutOff1, lightType1),
	light(lightPos2, lightDir2, lightAmb2, lightDif2, lightSpec2, spotDir2, spotExp2, spotCutOff2, lightType2),
	light(lightPos3, lightDir3, lightAmb3, lightDif3, lightSpec3, spotDir3, spotExp3, spotCutOff3, lightType3));
	vec4 color;
	...
	for(int i=0; i<3; i++){
		...
		if(NdotL>0.0) Specular = att*mymaterial.specular*lights[i].specular*pow(max(dot(R,V),0),mymaterial.shininess);
